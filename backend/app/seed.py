...
def init_db():
    ...
    CREATE TABLE IF NOT EXISTS repricing_rules(
        ...
        in_window_rate REAL NOT NULL, out_window_rate REAL NOT NULL,
    ...
    if conn.execute("SELECT COUNT(*) c FROM loans").fetchone()["c"] == 0:
        ...
        conn.execute("INSERT INTO repricing_rules(start_month,start_day,end_month,end_day,in_window_rate,out_window_rate,enabled,created_at,updated_at)"
                     " VALUES (11,1,2,28,3.10,3.60,1,datetime('now'),datetime('now'))")
